...
# Load background image with relative path
print("Attempting to load background image...")
try:
    background_image = pygame.image.load(os.path.join(ASSETS_DIR, "bg.jpg")).convert()
...
class Player:
    def __init__(self, start_y):
        try:
            self.base_image = pygame.image.load(os.path.join(ASSETS_DIR, "sol.png")).convert_alpha()
            self.width = 40
            self.height = 40
...
def start_screen():
    ...
    title = font.render("Trench Jumper", True, BLUE)
